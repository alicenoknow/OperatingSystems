...
void performTest(int size, char* sequance, FILE* fr){
	...
	void *handle = dlopen("libfirstlib.so", RTLD_LAZY);
	if (!handle){
		perror(dlerror());
		exit(1);
	}
	
	MainArrayOfBlocks* (*d_createMainArray)(int);
	char** (*d_mergeFiles)(int, char*);
	int (*d_createBlock)(MainArrayOfBlocks*, char*);
	void (*d_removeBlock)(MainArrayOfBlocks*, int);
	void (*d_cleanAll)(MainArrayOfBlocks*);
		
	d_createMainArray = dlsym(handle,"createMainArray");
	d_mergeFiles = dlsym(handle,"mergeFiles");
	d_createBlock = dlsym(handle,"createBlock");
	d_removeBlock = dlsym(handle,"removeBlock");
	d_cleanAll = dlsym(handle,"cleanAll");
	...
	MainArrayOfBlocks* arr = d_createMainArray(size);
	char** tmp;
	int idx;
	...
	tmp = d_mergeFiles(size, sequance);
	...
	for(int i = 0; i < 2 * size; i++){
		idx = d_createBlock(arr, tmp[0]);
		d_removeBlock(arr, idx);
	}
